...
export function Layout({ children }: { children?: React.ReactNode }) {
  ...
  const isCadastrosRouteActive = cadastrosItems.some(item => location === item.to);
      ...
      >
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
          ...
          {hasPermission(dashboardItem.permission) && (
            <Link
              href={dashboardItem.to}
              className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${
                location === dashboardItem.to
          ...
          {hasPermission(refuelingsItem.permission) && (
            <Link
              href={refuelingsItem.to}
              className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${
                location === refuelingsItem.to
                  ? 'bg-sidebar-accent text-sidebar-primary font-medium'
                  : 'text-sidebar-foreground hover:bg-sidebar-accent/50'
              }`}
            >
