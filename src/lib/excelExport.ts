...
export const exportVehicleRefuelingsToExcel = (
  ...
  const data = refuelings.map(r => {
    ...
    return {
      ...
      'KM': r.km || '-',
      'Combustível': r.fuelType,
      'Litros': new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(typeof r.liters === 'string' ? parseFloat(r.liters) : r.liters),
      'Preço/Litro (R$)': new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(typeof r.pricePerLiter === 'string' ? parseFloat(r.pricePerLiter) : r.pricePerLiter),
      'Valor Total (R$)': new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(typeof r.totalValue === 'string' ? parseFloat(r.totalValue) : r.totalValue),
